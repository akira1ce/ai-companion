import { HistoryMessage } from "@/services/type";

/**
 * 判断两条消息是否属于同一组（同发送者且间隔 <60s）
 */
export function isSameGroup(a: HistoryMessage, b: HistoryMessage): boolean {
 ...
 * 将扁平消息数组分组
 */
export function groupMessages(messages: HistoryMessage[]): MessageGroup[] {
